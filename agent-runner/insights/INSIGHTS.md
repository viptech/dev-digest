...
**`run.test.ts`'s shared event-payload fixture не мала `pull_request.head.sha` — додавання required `PrContext.headSha` (SPEC-08 T10) ламало б усі тести, що йдуть через `resolvePrContext`, якби фікстуру не оновили разом зі схемою**
`resolvePrContext` (`context.ts:93-98`) кидає `RunnerError`, якщо ані
`event.pull_request.head.sha`, ані `env.GITHUB_SHA` не задані — а спільний
`beforeEach`-фікстур (`run.test.ts:159-170`, до цієї сесії) писав `head: {
repo: { fork: false } }` без `sha` взагалі, і `baseDeps()`
(`run.test.ts:182-196`) ніде не встановлював `GITHUB_SHA`. Без правки
фікстури кожен тест, що доходить до `resolvePrContext` (усі, крім AC-20,
який провалюється раніше на валідації маніфесту), почав би падати з новою
`RunnerError` замість очікуваного успіху/помилки за сценарієм. Фікс:
додано `sha: '<40-char>'` до фікстурного `pull_request.head`
(`run.test.ts:167`) — той самий паттерн, що вже задокументований у
`server/INSIGHTS.md` (2026-08-03: required-поле в контракті ламає
неоновлену фіксту без помилки компілятора) застосовується й тут, лише в
`agent-runner`. **Не підтверджено фактичним прогоном тестів** (сесія без
`Bash`-інструменту) — перевір `pnpm test` в `agent-runner/` перш ніж
довіряти цій правці на 100%.
Доказ: agent-runner/src/context.ts:93-98; agent-runner/src/run.test.ts:167

## Open Questions

~~2026-07-08 — `AgentManifest` has no `post_as` field and the generated GHA workflow (`server/src/modules/ci/workflow.ts`) doesn't set a `POST_AS`-equivalent env var...~~ **RESOLVED 2026-08-22 (SPEC-08 Group 2/coordinator).** Threaded through the workflow as an explicit env var, not folded into `AgentManifest`: `ci/workflow.ts`'s `generateWorkflowYaml()` now bakes `env: { DEVDIGEST_POST_AS: '<configured post_as>' }` as a plain static string into the generated `.github/workflows/devdigest-review.yml` — never a secret, never templated from untrusted input, set from the Export Wizard's Configure-step choice at export time. `index.ts`'s existing `DEVDIGEST_POST_AS` env-var read (line 25, unchanged) is what actually consumes it — this closes the loop entirely on the generator side, no `runCi()`/`index.ts` changes needed. ref: server/src/modules/ci/workflow.ts:52-58,60-98 (`generateWorkflowYaml`); docs/specs/SPEC-08-export-to-ci.md AC-11
