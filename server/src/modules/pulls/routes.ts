...
export default async function pullsRoutes(appBase: FastifyInstance) {
  ...
  app.get('/repos/:id/pulls', { schema: { params: IdParams } }, async (req): Promise<PrMeta[]> => {
    ...
    // Total spend per PR — SUM(cost_usd) across ALL its agent_runs (every
    // review pass, not just the latest), for the list's COST column.
    // drizzle's sum() returns a numeric string (or null when there are no
    // matching rows / all values are null); coerce to a number.
