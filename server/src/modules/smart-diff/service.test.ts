...
describe('assembleSmartDiff (pure)', () => {
  it('groups files by role and attaches sorted finding_lines per file', () => {
    const files = [
    ...
    const findings = [
      finding({ file: 'src/service.ts', startLine: 30 }),
      finding({ file: 'src/service.ts', startLine: 12 }),
      finding({ file: 'src/index.ts', startLine: 5 }),
    ];
    ...
    expect(wiring.files[0]).toMatchObject({ path: 'src/index.ts', finding_lines: [5] });

    const boilerplate = result.groups.find((g) => g.role === 'boilerplate')!;
    expect(boilerplate.files[0]).toMatchObject({ path: 'package-lock.json', finding_lines: [] });
  });

  it('degrades to finding_lines: [] for every file when no review has run yet', () => {
...
describe('SmartDiffService.build (assembly with a stub repository, no DB)', () => {
  it('never touches any LLM — only the repository port is called', async () => {
    ...
    expect(result.groups.find((g) => g.role === 'core')!.files[0]!.finding_lines).toEqual([12]);
  });

  it('degrades to finding_lines: [] when the findings lookup rejects (no review yet)', async () => {
    const repo: SmartDiffRepo = {
      getPrFiles: vi.fn(async () => [prFile({ path: 'src/service.ts' })]),
    ...
    const result = await service.build('pr-1');
    expect(result.groups.find((g) => g.role === 'core')!.files[0]!.finding_lines).toEqual([]);
  });
});
