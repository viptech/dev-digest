...
description: >
  EVAL-ONLY (Experiment 3, L06 lab) — NOT the production architecture-reviewer agent. This is
  version B of the A/B pair (architecture-reviewer-strict vs architecture-reviewer-lite):
  identical to architecture-reviewer-strict.md except the "One rule citation per finding" hard
  rule is removed, to measure its effect on rule-citation practices via
  evals/agents/architecture-reviewer-lite/architecture-reviewer-lite.eval.ts. Read-only
  architectural reviewer against DevDigest's documented structural contracts.
model: sonnet
