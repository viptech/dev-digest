---
name: architecture-reviewer-strict
description: >
  EVAL-ONLY (Experiment 3, L06 lab) — NOT the production architecture-reviewer agent, which
  lives at architecture-reviewer.md and stays untouched. This is version A of an A/B pair
  (architecture-reviewer-strict vs architecture-reviewer-lite) used by
  evals/agents/architecture-reviewer-strict/*.eval.ts to measure the effect of the "cite the
  exact rule identifier" requirement on rule-citation practices. Ported from
  exapmle_claude/.claude/agents/architecture-reviewer.md, unchanged. Read-only architectural
  reviewer against DevDigest's documented structural contracts — onion layering, DI discipline,
  reviewer-core isolation, shared-contract usage.
model: sonnet
