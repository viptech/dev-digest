...
export const s = {
  card: (focused: boolean, sevColor: string, muted: boolean): CSSProperties => ({
    borderRadius: 8,
    // All-longhand, including per-side color (never mix `border`/`borderColor`
    // with `borderLeft*` — React warns about updating a multi-side shorthand
    // together with a side-specific longhand on the same rerender).
    borderStyle: "solid",
